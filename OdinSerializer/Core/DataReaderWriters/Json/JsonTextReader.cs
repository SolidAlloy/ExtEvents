                ...
                if (insideString && lastReadChar == '\\')
                {
                    ...
                    switch (c) // '\"' is handled further down
                    {
                        ...
                        case 'u':
                            ...
                            Context.Config.DebugContext.LogError("A wild non-hex value appears at position " + reader.BaseStream.Position + "! \\-u-" + c1 + "-" + c2 + "-" + c3 + "-" + c4 + "; current buffer: '" + new string(buffer, 0, bufferIndex + 1) + "'. If the error handling policy is resilient, an attempt will be made to recover from this emergency without a fatal parse error...");
        ...
        private void ParseEntryFromBuffer(out string name, out string valueContent, out EntryType entry, int valueSeparatorIndex, EntryType? hintEntry)
        {
            if (bufferIndex >= 0)
            {
                ...
                if (valueContent != null)
                {
                    ...
                    if (valueContent.StartsWith(JsonConfig.INTERNAL_REF_SIG, StringComparison.InvariantCulture))
                    {
                        entry = EntryType.InternalReference;
                        return;
                    }

                    if (valueContent.StartsWith(JsonConfig.EXTERNAL_INDEX_REF_SIG, StringComparison.InvariantCulture))
                    {
                        entry = EntryType.ExternalReferenceByIndex;
                        return;
                    }

                    if (valueContent.StartsWith(JsonConfig.EXTERNAL_GUID_REF_SIG, StringComparison.InvariantCulture))
                    {
                        entry = EntryType.ExternalReferenceByGuid;
                        return;
                    }

                    if (valueContent.StartsWith(JsonConfig.EXTERNAL_STRING_REF_SIG_OLD, StringComparison.InvariantCulture))
                    {
                        entry = EntryType.ExternalReferenceByString;
                        return;
                    }

                    if (valueContent.StartsWith(JsonConfig.EXTERNAL_STRING_REF_SIG_FIXED, StringComparison.InvariantCulture))
                    {
                        entry = EntryType.ExternalReferenceByString;
                        return;
                    }
